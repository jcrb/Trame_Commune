...
if(test == 0){
    file <- "RPU_2014/rpu2014d0112_c2.Rda" # données de 2014
    load(paste0(path, file))
    dx <- d14 # si file actif
    rm(d14)
}else{
    file <- "RPU_2014/rpu2015d01.Rda" # fichier de test
    load(paste0(path, file))
    dx <- d01 
    rm(d01)
}
...
#### Exhaustivité quantitative régionale (NB de RPU/Nb de passages x Nb SU transmettant des RPU/Nb total SU): `r format((n.rpu * 100 / passages.2013.SAE) * (15 * 100/16)/100)` %.
...
# Résultat sauvegardé pour tout 2014 dans horaires_pds.R
pop75$pds <- sapply(pop75$ENTREE, pds)
s.pds <- summary(as.factor(pop75$pds))
