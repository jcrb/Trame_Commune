...
load(paste0(path, file_actif)) # d14 = données 2014 corrigées
...
# barplot
barplot(sort(completude), horiz = TRUE, las = 1, cex.names = 0.8, xlab = "% de complétude", main = "Complétude des champs RPU", col = "light green")
abline(v = 90, lty = 2, col = "red")

# diagramme en étoile
radial.plot(completude, rp.type="p", radial.lim=c(0,100), start = 1.57, clockwise = TRUE, line.col = "red", labels = names(completude), cex.axis = 0.6)
...
mean.rpu.quot <- n.rpu / 365 # Moyenne quotidienne de passages

chiffres2014 <- c(anc, n.su.ped, n.su.adultes, n.su.polyvalents, n.su.publics, n.su.prives, n.rpu, mean.rpu.quot)
```
...
- nombre de SU dans le secteur public: `r n.pub`
- nombre de SU dans le secteur privé: `r n.prv`

## Nombre de passages dans l'année [C]

`r format(n.rpu, big.mark = " ")`

## Moyenne quotidienne de passages [C]

`r format(mean.rpu.quot, big.mark = " ")`
...
m.rpu.sem <- sum(n.par.wday[2:6])/5 # moyenne des RPU en semaine
m.rpu.we <- sum(n.par.wday[1], sum(n.par.wday[7]))/2 # moyenne RPU le WE
p.activite.we <- 100 * (m.rpu.we - m.rpu.sem) / m.rpu.we
```
...
# horaires seuls. Li faut isoler les heures de la date
...
`r round(n.passages.nuit * 100 / n.passages, 2)` % (N = `r n.passages.nuit`)
...
`r round(n.passages.nuit.profonde * 100 / n.passages, 2)` % (N = `r n.passages.nuit.profonde`)
...
n2014 <- nrow(dx) # nb de RPU = nb passages
mean.rpu.jour.2014 <- n2014 / 365 # moyenne quotidienne pour 2014

# n2014 <- n
n2013 <- 344073 # nb de RPU pour 2013
...
x <- barplot(c(n2013, n2014), names.arg = annee, ylab = "nombre de RPU", main = "Evolution du nombre deRPU en Alsace", xlim = lim, col="light green", axes=FALSE) # x = abcisse du centre des barres verticales
text(x[2,1], n2014 * 0.1, "+ 21%")
axis(2, cex.axis = .7, las = 2)

par(new=TRUE) # voir http://cran.r-project.org/doc/contrib/Lemon-kickstart/kr_addat.html 

xx <- cbind(x, c(mean.rpu.jour.2013, mean.rpu.jour.2014)) # matrice des coordonnées x et y des points formant la droite des moyennes des RPU.
...
moyenne d'age: `r mean.age` ans, ecart-type: TODO
