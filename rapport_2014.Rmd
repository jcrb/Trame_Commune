...
- Nombre total de RPU / Nb total de passages SAE (Statistique annuelle des établissements de santé) (suivant disponibilité): `r format.n(n.rpu * 100 / passages.2013.SAE)` % (NOTE le nombre de passages déclarés est celui indiqué par les données SAE 2013)

- Exhaustivité quantitative régionale (NB de RPU/Nb de passages x Nb SU transmettant des RPU/Nb total SU): 
`r format((n.rpu * 100 / passages.2013.SAE) * (15 * 100/16)/100)` %.
...
- Nombre de passages dans l'année [C]: __`r format.n(n.rpu)` RPU__
- Moyenne quotidienne de passages [C]: __`r format.n(mean.rpu.quot)` RPU__
- %(N) d'évolution par rapport à année N-1 [C]: __`r format((n.rpu / n.rpu.2013 - 1)*100)` %__.
...
- Données renseignées (données à partir desquelles tout le reste de l’analyse sera effectuée)
    - Nombre de RPU transmis: __`r format.n(n.rpu)` RPU__
    - Exhaustivité du recueil : Nb RPU transmis / Nb de passages déclarés __`r format.n(n.rpu * 100 / passages.2013.SAE)` %__ (NOTE le nombre de passages déclarés est celui indiqué par les données SAE 2013)
...
# archivage
val <- c(n.pub, n.prv, p.pub, p.prv)
names(val) <- c("n.pub", "n.prv", "p.pub", "p.prv")
...
chiffres <- c(chiffres, val)
```
- %(N) Femme [C] __`r round(p.femme, 2)` %__ (`r format(sexe['F'], big.mark = " ")`)
- %(N) Homme [C] __`r round(p.homme, 2)` %__ (`r format(sexe['M'], big.mark = " ")`)
...
# archivage
val <- c(n.inf1an, n.supegal90, p.inf1an, n.inf18an, p.inf18an, n.supegal75an, p.supegal75an, mean.age, sd.age, median.age, mean.age.h, mean.age.f)
...
- % (N) < 1 an [C] __`r round(100 * p.inf1an, 2)` %__ (`r n.inf1an`)
- %(N) < 18 ans [C] __`r round(100 * p.inf18an, 2)` %__ (`r n.inf18an`)
- %(N) >= 75 ans [C] __`r round(100 * p.supegal75an, 2)` %__ (`r n.supegal75an`)
- Age moyen[C]: __`r round(mean.age, 2)` ans__.
...
annee <- c(anp, anc) # vecteur des noms des années
...
xx <- cbind(x, c(mean.rpu.jour.anp, mean.rpu.jour.anc)) # matrice des coordonnées x et y des points formant la droite des moyennes des RPU.
...
file <- "../../Stat Resural/RPU_Doc/Carto/Hopitaux_Alsace.csv"
hop <- read.csv( file)
...
# TRES LONG: il faut appliquer à chaque entrée la précédure pds (rapport_2014.R)
# pop75$psd <- NA
# pop75$pds <- sapply(pop75$ENTREE, pds)
...
- %  passages la nuit: `r nuit[2]*100` % (N = `r nuit[1]`)
- %  passages en horaire de PDS: `r p.pds` % (N = `r n.pds`)
- %  d’arrivées Moyen perso: `r p.moy.perso.75` % (N = `r n.moy.perso.75`)
- %  d'arrivées SMUR: `r p.moy.smur.75` % (N = `r n.moy.smur.75`)
- %  d'arrivées VSAV: `r p.moy.vsab.75` % (N = `r n.moy.vsab.75`)
- %  d'arrivées ambulance privée: `r p.moy.ambu.75` % (N = `r n.moy.ambu.75`)
- % réponses manquantes: 
...
- %  CCMU 1: `r p.ccmu75.1` % (N = `r n.ccmu75.1`)
- %  CCMU 4 et 5: `r p.ccmu75.45` % (N = `r n.ccmu75.45`)
...
- %  d’hospitalisation: `r p.hosp.75` % (N = `r n.hosp.75`)
- % de mutation:`r p.mut.75` % (N = `r n.mut.75`)
- % de transfert:`r p.trans.75` % (N = `r n.trans.75`)
- %  de retour à domicile:`r p.dom.75` % (N = `r n.dom.75`)
...
a <- cut(duree/60, duree.c, right = FALSE) # il faut transformer les durées en heures.
...
```{r chiffres_cles, echo=FALSE, comment=""}
# format.n(chiffres)
options(scipen = 6, digits = 2)
chiffres
```
