...
ped.passage4 <- length(ped.dp$duree[ped.dp$duree < 4 * 60 + 1]) #nb passages < 4h
s.ped.mode.sortie <- summary(ped.dp$MODE_SORTIE)
ped.hosp.passage4 <- length(ped.dp$duree[ped.dp$duree < 4 * 60 + 1 & ped.dp$MODE_SORTIE %in% c("Mutation","Transfert")]) #nb passages < 4h et hospitalisation

ped.dom.passage4 <- length(ped.dp$duree[ped.dp$duree < 4 * 60 + 1 & ped.dp$MODE_SORTIE == "Domicile"]) #nb passages < 4h et retourà domicile
...
- Nombre d’AVC ischémique et %: `r format.n(dp.avc[1,3])` (`r dp.avc[2,3]` %)
- Nombre d’AIT et %: `r format.n(dp.avc[1,1])` (`r dp.avc[2,1]` %)
- Nombre de codes "symptomatiques" (hémiplégie, aphasie, amaurose, etc…) et %
- Nombre d’autres hémorragies non traumatiques et %: `r format.n(dp.avc[1,2])` (`r dp.avc[2,2]` %)
...
n.ts.avc.inf4h <- length(ts.avc < 4 * 60) # nombre AVC restés mons de 4 heures aux urgences
...
- % d’hospitalisation: `r avc.sortie$output.table[1,3] + avc.sortie$output.table[2,3]` %
- % de mutation: `r avc.sortie$output.table[1,3]` %
- % de transfert: `r avc.sortie$output.table[2,3]` %
- % de retour à domicile: `r avc.sortie$output.table[3,3]` %
