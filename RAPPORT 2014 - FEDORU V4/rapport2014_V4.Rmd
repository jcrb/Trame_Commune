...
Ajout de la dernière versin de Gilles 
...
- nombre de logicels et ombre de SU par région
- SAE 2014 ?
...
names(val) <- c("nb_etrangers", "p_etrangers")
chiffres <- c(chiffres, val)
#---------------------- Chiffres ---------------------------------------------------------------
```
...
p.pds <- p.s.pds["PDSS"] + p.s.pds["PDSWE"] # nombre de passages aux haoraires de PDS
...
- %  passages en horaire de PDS: `r p.pds * 100` % (Remarque: ne tient pas compte des jours fériés survenant en semaine)
